...
This value defines the minimum time between the press and release of the tap behavior's key down/key up events, in milliseconds. By default this is 0, meaning the tap behavior is released as soon as it would otherwise be (e.g. immediately after being pressed if the hold-tap key itself was released quickly).

Raising this value can help when the host or an application doesn't reliably register very short key presses.
